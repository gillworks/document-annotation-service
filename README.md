...
### Redaction agent (paired with the detection agent)

The shipped pipeline detects sensitive information at the schema level (`pii_detected.present/types/count`) but stops short of acting on it. The production extension adds a second artifact per job: a redacted copy of the original document with sensitive spans permanently destroyed — not visually overlaid, because the underlying text must be removed before fill is applied or the redaction is copy-paste-defeatable, the source of well-known real-world redaction failures. A two-agent flow makes this safe enough for regulated industries: the detection agent locates each sensitive value with a verified citation; the redaction agent consumes those citations, produces the redacted output, and hands off to an isolated critic agent (per the critic-agent design above) that re-scans the redacted artifact and asserts no listed PII type is still present. The same `pending_review` state from the HITL design gates low-confidence redactions to a human reviewer before the file is released. Output is stored as `redacted_storage_path` alongside the original, with a full audit log of what was redacted, where, and why. Implementation depends on extractor work this v1 deliberately doesn't do — bbox-aware PDF extraction and cell-reference-aware spreadsheet extraction — which makes this the largest designed-but-not-shipped item here, and the natural seam where the product shifts from "extract" to "transform."
